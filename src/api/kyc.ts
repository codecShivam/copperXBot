...
export const getKycStatus = async (token: string) => {
  try {
    ...
    const response = await axios.get(`${config.api.baseURL}/kycs?limit=1`, {
      headers: {
 ...
 */
export const getWithdrawalFeeEstimate = async (
  amount: string,
  currency: string,
  country?: string,
) => {
  try {
    console.log(
      `[API] Getting fee estimate for ${amount} ${currency}, country: ${country || 'not specified'}`,
    );
    ...
    const coinId =
      currencyMapping[currency.toUpperCase()] || currency.toLowerCase();
    ...
    if (priceResponse.data && priceResponse.data[coinId]) {
      ...
      console.log(
        `[API] Could not get price for ${currency}, using 1 USD as fallback`,
      );
      ...
      try {
        ...
        if (
          usdInrResponse.data &&
          usdInrResponse.data.tether &&
          usdInrResponse.data.tether.inr
        ) {
        ...
        console.log(
          `[API] Could not get USD to INR rate, using default value: ₹83`,
        );
      }
    }
    ...
    const isIndianUser =
      country &&
      (country.toLowerCase() === 'ind' || country.toLowerCase() === 'india');
