...
import { Wallet, WalletWithBalances } from '../types';
...
export const getBalances = async (
  ...
  try {
    ...
    // Determine if the response is directly an array or has a .data property
    const walletBalances = Array.isArray(response.data)
      ? response.data
      : response.data.data || [];
    console.log(
      `[WALLET API] Successfully fetched balances for ${walletBalances.length} wallets`,
    );
...
export const setDefaultWallet = async (
  ...
  try {
    ...
    console.error(
      `[WALLET API] Error setting default wallet ${walletId}:`,
      error,
    );
    if (axios.isAxiosError(error) && error.response) {
      console.error('[WALLET API] API error response:', error.response.data);
...
export const generateWallet = async (
  ...
  console.log(
    `[WALLET API] Generating new wallet for network ID: ${networkId}`,
  );
  try {
    console.log(
      `[WALLET API] Sending POST request to /wallets with network: ${networkId}`,
    );
    ...
    console.log(
      `[WALLET API] New wallet generated successfully for network ID: ${networkId}`,
    );
    console.log(
      `[WALLET API] Raw API response:`,
    ...
    if (response.data && typeof response.data === 'object') {
      if (response.data.hasOwnProperty('data')) {
        ...
        response.data.hasOwnProperty('walletAddress') ||
    ...
    console.error(
      `[WALLET API] Error generating wallet for network ID ${networkId}:`,
      error,
    );
    if (axios.isAxiosError(error) && error.response) {
      console.error('[WALLET API] API error response:', error.response.data);
      throw new Error(
        error.response.data.message ||
          `Failed to generate wallet for network ID ${networkId}`,
      );
    }
