...
const loginCommand = Composer.command('login', async (ctx) => {
  ...
  await ctx.reply('🔐 *Login to Copperx*\n\nPlease enter your email address:', {
    parse_mode: 'Markdown',
  });
...
const loginFlow = Composer.on(message('text'), async (ctx, next) => {
  ...
  if (session.currentStep === 'login_otp') {
    ...
    if (!/^\d+$/.test(text)) {
      ...
      await ctx.reply(
        '❌ Invalid OTP format. Please enter a numeric OTP code:',
      );
      return;
    }
    ...
    try {
      ...
      if (!sid) {
        ...
        throw new Error(
          'Session ID not found. Please restart the login process.',
        );
      }
      ...
      try {
        ...
        const refreshToken =
          response.refreshToken || accessTokenId || accessToken;
