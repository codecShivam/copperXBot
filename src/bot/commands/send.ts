...
async function estimateFees(
  ...
  try {
    ...
    if (transferType !== 'bank') {
      console.log(
        `[SEND] No fee estimation needed for ${transferType} transfers`,
      );
    ...
    console.log(
      `[SEND] Estimating bank withdrawal fees for ${amount} ${currency}`,
    );
    ...
    const estimatedReceiveAmount =
      estimatedReceiveValue > 0 ? estimatedReceiveValue.toFixed(2) : '0.00';
...
const sendEmailAction = Composer.action(
  'send_email',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const sendWalletAction = Composer.action(
  'send_wallet',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const sendFlow = Composer.on(message('text'), async (ctx, next) => {
  ...
  if (session.currentStep === 'send_wallet_recipient') {
    ...
    try {
      ...
      const balances = await walletApi.getBalances(session.token as string);
      console.log(
        `[SEND] Fetched ${balances.length} wallets with balances for address validation`,
      );
      ...
      for (const network of networks) {
        ...
        if (isValid) {
          ...
          console.log(
            `[SEND] Address ${text.substring(0, 10)}... is valid for ${network}`,
          );
        }
      }
      ...
      if (!isValidForAnyNetwork) {
        console.log(
          `[SEND] Address ${text.substring(0, 10)}... is not valid for any network: ${networks.join(', ')}`,
        );
        await ctx.reply(
          "❌ Invalid wallet address. The address format doesn't match any of your available networks. Please enter a valid address:",
        );
        return;
      }
      ...
      if (validNetworks.length > 0) {
        console.log(
          `[SEND] Address ${text.substring(0, 10)}... is valid for networks: ${validNetworks.join(', ')}`,
        );
      }
  ...
  if (session.currentStep === 'send_network') {
    ...
    try {
      ...
      console.log(
        `[SEND] Fetched ${walletData.length} wallets with balances for token selection`,
      );
  ...
  if (session.currentStep === 'send_token') {
    ...
    console.log(
      `[SEND] Selected token ${selectedToken} on ${formatNetworkForDisplay(selectedNetwork)}`,
    );
  ...
  if (session.currentStep === 'send_amount') {
    ...
    try {
      ...
      console.log(
        `[SEND] User entered amount: ${amount} - will be multiplied by 10^8 in API layer`,
      );
      ...
      await ctx.reply(
        '❌ Invalid amount format. Please try a different amount:',
      );
    }
    return;
  }
...
async function displayTransactionConfirmation(ctx) {
  ...
  try {
    ...
    if (transferType === 'email') {
      ...
      const feeEstimate = await estimateFees(amount, token, transferType);
...
const confirmAmountAction = Composer.action(
  'confirm_amount',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const sendConfirmAction = Composer.action(
  'send_confirm',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
    ...
    try {
      ...
      async function checkKycStatus(): Promise<{
        isApproved: boolean;
        status?: string;
        message?: string;
      }> {
      ...
      console.log(
        `[SEND] Initiating ${transferType} transfer of ${amount} ${token} (raw amount) to ${recipient}`,
      );
      ...
      if (transferType === 'email') {
        // Send to email
        response = await transfersApi.sendEmailTransfer(
          getSession(ctx).token as string,
          {
            amount,
            token,
        ...
        // Send to wallet
        response = await transfersApi.sendWalletTransfer(
          getSession(ctx).token as string,
          {
            amount,
            token,
...
const sendCancelAction = Composer.action(
  'send_cancel',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const sendBatchAction = Composer.action(
  'send_batch',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const batchFlowHandler = async (ctx, next) => {
  ...
  if (session.currentStep === 'batch_network') {
    ...
    if (
      ...
      await ctx.reply(
        `${ICON.error} Invalid selection. Please enter a number between 1 and ${networks.length}:`,
      );
      return;
    }
  ...
  if (session.currentStep === 'batch_add_recipients') {
    ...
    if (upperText === 'DONE') {
      if (batchState.payees.length === 0) {
        await ctx.reply(
          `${ICON.warning} No recipients added. Please add at least one recipient or type CANCEL to abort.`,
        );
        return;
      }
      ...
      setTempData(ctx, 'batchState', batchState);
      await ctx.reply(
        `${ICON.success} All recipients cleared. You can now add new recipients.`,
      );
      return;
    ...
    if (parts.length < 2) {
      await ctx.reply(
        `${ICON.error} Invalid format. Please use: email@example.com 10.5`,
      );
      return;
    }
    ...
    if (!validateEmailFormat(email)) {
      await ctx.reply(
        `${ICON.error} Invalid email format. Please enter a valid email address.`,
      );
      return;
    }
    ...
    if (isNaN(amount) || amount <= 0) {
      await ctx.reply(
        `${ICON.error} Invalid amount. Please enter a positive number.`,
      );
      return;
    }

    // Check for duplicate email
    const existingIndex = batchState.payees.findIndex(
  ...
  if (session.currentStep === 'batch_confirm_update') {
    ...
    if (response === 'YES') {
      ...
      await ctx.reply(
        `${ICON.error} Invalid response. The entry was not updated.`,
      );
    }
...
const batchConfirmAction = Composer.action(
  'batch_confirm',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
    ...
    try {
      ...
      const response = await transfersApi.sendBatchPayments(
        session.token,
        batchRequests,
      );
...
const batchCancelAction = Composer.action(
  'batch_cancel',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
