...
const balanceCommand = Composer.command(
  'balance',
  authMiddleware(),
  async (ctx) => {
...
const balanceAction = Composer.action(
  'balance',
  authMiddleware(),
  async (ctx) => {
...
const allBalancesAction = Composer.action(
  'balance_all',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const refreshBalancesAction = Composer.action(
  'balance_refresh',
  authMiddleware(),
  async (ctx) => {
...
const networkSelectionAction = Composer.action(
  'balance_networks',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const walletDetailsAction = Composer.action(
  'wallet_details',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const walletSettingsAction = Composer.action(
  'wallet_settings',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.reply(
      formatHeader('Wallet Settings') +
        DIVIDERS.medium +
...
const setDefaultWalletAction = Composer.action(
  'wallet_set_default',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const viewWalletAddressAction = Composer.action(
  'wallet_view_address',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const tokenDetailsAction = Composer.action(
...
const generateWalletAction = Composer.action(
  'wallet_generate',
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
async function showNetworkSelectionForWalletGeneration(ctx) {
  try {
    const message =
      formatHeader('Generate New Wallet') +
      DIVIDERS.medium +
    ...
    console.error(
      'Error showing network selection for wallet generation:',
      error,
    );
    await ctx.reply(
    ...
    await ctx.reply(
      `${ICON.back} Return to wallet settings:`,
...
const generateWalletNetworkAction = Composer.action(
  /^generate_wallet_(.+)_(.+)$/,
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
    ...
    console.log(
      `[BALANCE] Generate wallet for network ${networkName} (ID: ${networkId}) action triggered`,
    );
...
const networkPaginationAction = Composer.action(
  /^network_page_(\d+)$/,
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const networkBalancesAction = Composer.action(
  /^balance_network_(.+)$/,
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const tokenDetailsViewAction = Composer.action(
  /^token_details_(.+)_(.+)$/,
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
const walletSelectionAction = Composer.action(
  /^set_default_(.+)$/,
  authMiddleware(),
  async (ctx) => {
    await ctx.answerCbQuery();
...
async function getCachedBalances(ctx): Promise<{
  ...
  walletData.forEach((wallet) => {
    ...
    if (wallet.balances && Array.isArray(wallet.balances)) {
      console.log(
        `[BALANCE] Processing ${wallet.balances.length} token balances`,
      );
      wallet.balances.forEach((balance) => {
        ...
        console.log(
          `[BALANCE] Token: ${balance.symbol}, Balance: ${balance.balance}`,
        );
      ...
      console.log(
        `[BALANCE] No balances found for wallet on network ${network}`,
      );
    }
  });
...
async function fetchAndDisplayBalances(ctx, isRefresh = false) {
  try {
    if (!isRefresh) {
      await ctx.reply(formatLoading('Fetching your wallet balances...'), {
        parse_mode: 'Markdown',
      });
    }
...
async function showNetworkBalances(ctx, network: string) {
  try {
    ...
    if (networkBalances.length === 0) {
      ...
      await ctx.reply(
        'Return to networks:',
        backButtonKeyboard('balance_networks'),
      );
      return;
    }
...
async function showTokenDetails(ctx, network: string, token: string) {
  try {
    ...
    if (!tokenBalance) {
      ...
      await ctx.reply(
        'Return to network balances:',
        backButtonKeyboard(`balance_network_${network}`),
      );
      return;
    }
    ...
    const actionButtons = Markup.inlineKeyboard([
      [
        Markup.button.callback(
          `${ICON.send} Send`,
          `send_init_${network}_${token}`,
        ),
      ],
      [
        Markup.button.callback(
          `${ICON.withdraw} Withdraw`,
          `withdraw_init_${network}_${token}`,
        ),
      ],
      [
        Markup.button.callback(
          `${ICON.back} Back to ${formatNetworkForDisplay(network)}`,
    ...
    await ctx.reply(
      `Return to ${formatNetworkForDisplay(network)} balances:`,
      backButtonKeyboard(`balance_network_${network}`),
    );
  }
}
...
async function showWalletDetails(ctx) {
  try {
    await ctx.reply(formatLoading('Fetching wallet details...'), {
      parse_mode: 'Markdown',
    });
...
async function showWalletAddresses(ctx) {
  try {
    await ctx.reply(formatLoading('Fetching your wallet addresses...'), {
      parse_mode: 'Markdown',
    });
...
async function showSetDefaultWallet(ctx) {
  try {
    ...
    const wallets = await walletApi.getWallets(token);
    console.log(
      `[BALANCE] Fetched ${wallets.length} wallets for default selection`,
    );
...
async function generateNewWallet(ctx, networkId: string, networkName: string) {
  try {
    await ctx.reply(
      formatLoading(
        `Generating a new wallet on ${formatNetworkForDisplay(networkName)}...`,
      ),
      { parse_mode: 'Markdown' },
    );
    ...
    let message =
      formatSuccess('Wallet generated successfully!') + DIVIDERS.medium;
