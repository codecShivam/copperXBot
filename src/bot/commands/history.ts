...
const userViewState = new Map<string, {
  currentPage: number,
...
function mapTransactionForUI(transaction) {
  ...
  return {
    ...
    network: transaction.fromAccount?.network || transaction.toAccount?.network || 'unknown'
  };
}
...
async function fetchAndDisplayHistory(ctx, page = 1) {
  try {
    ...
    if (page === 1) {
      await ctx.reply(
        formatLoading('Fetching your transaction history...'),
        { parse_mode: 'Markdown' }
      );
    }
    ...
    console.log(`[HISTORY] Making API request to /transactions with page=${page}, limit=${ITEMS_PER_PAGE}`);
    
    // Fetch transactions with pagination only (no status filter)
    const response = await transfersApi.getTransferHistory(token, page, ITEMS_PER_PAGE);
    
    console.log(`[HISTORY] API response received:`, JSON.stringify(response, null, 2));
    
    // Validate response structure
    if (!response || typeof response !== 'object') {
      console.error('[HISTORY] Invalid response format - not an object:', response);
    ...
    console.log(`- Has data.data array: ${!!(response.data.data && Array.isArray(response.data.data))}`);
    console.log(`- Has data.count: ${typeof response.data.count !== 'undefined'}`);
    console.log(`- Has data.hasMore: ${typeof response.data.hasMore !== 'undefined'}`);
    console.log(`- Has data.page: ${typeof response.data.page !== 'undefined'}`);
    console.log(`- Has data.limit: ${typeof response.data.limit !== 'undefined'}`);
    ...
    if (typeof response.data.count === 'number') {
      ...
      console.log(`[HISTORY] Using response.data.count for total: ${totalItems}`);
    } else if (typeof (response.data as any).total === 'number') {
      totalItems = (response.data as any).total;
      console.log(`[HISTORY] Using response.data.total for total: ${totalItems}`);
    } else {
      // If no count found, estimate based on current page and items per page
      const estimatedTotal = (page - 1) * ITEMS_PER_PAGE + rawTransactions.length;
      totalItems = Math.max(estimatedTotal, rawTransactions.length);
      console.log(`[HISTORY] No count/total field found, estimating as ${totalItems}`);
    }
    ...
    if (totalItems < rawTransactions.length) {
      ...
      console.log(`[HISTORY] Adjusted total items to match transaction count: ${totalItems}`);
    }
    ...
    if (totalItems <= 0 && rawTransactions.length > 0) {
      ...
      console.log(`[HISTORY] Forced minimum total items to enable pagination: ${totalItems}`);
    }
    ...
    if (typeof response.data.hasMore === 'boolean') {
      ...
      if (page === 1 && rawTransactions.length >= ITEMS_PER_PAGE) {
        hasMore = true;
        console.log(`[HISTORY] On first page with full results, assuming more pages exist`);
      }
      
      console.log(`[HISTORY] No hasMore field found, calculated as ${hasMore} (page ${page} of ${totalPages})`);
    }
    
    // Force hasMore to true if we have a full page of results
    if (!hasMore && rawTransactions.length >= ITEMS_PER_PAGE) {
      console.log(`[HISTORY] Got full page of results (${rawTransactions.length}), forcing hasMore=true`);
      hasMore = true;
    }
    
    console.log(`[HISTORY] Extracted ${rawTransactions.length} transactions, totalItems=${totalItems}, hasMore=${hasMore}`);
    
    // Map transactions to UI format
    const transactions = rawTransactions.map(mapTransactionForUI);
    
    console.log(`[HISTORY] Mapped ${transactions.length} transactions for UI display`);
    console.log(`[HISTORY] First transaction sample:`, 
    ...
    console.error('[HISTORY] Error details:', error instanceof Error ? error.stack : 'Unknown error type');
    await ctx.reply(
...
async function displayTransactions(ctx, transactions, currentPage, totalItems) {
  ...
  transactions.forEach((transfer, index) => {
    try {
      ...
      const originalAmount = parseFloat(transfer.amount || transfer.fromAmount || "0");
      const divisor = Math.pow(10, 8);
      const adjustedAmount = originalAmount / divisor;
      const amount = formatAmount(adjustedAmount);
      
      console.log(`[HISTORY] Original amount: ${originalAmount}, Adjusted amount (÷10^8): ${adjustedAmount}`);
      
      const currency = escapeMarkdown(transfer.token || transfer.fromCurrency || 'unknown');
      const transferStatus = escapeMarkdown(transfer.status.charAt(0).toUpperCase() + transfer.status.slice(1));
      const type = escapeMarkdown(transfer.type.charAt(0).toUpperCase() + transfer.type.slice(1));
      
      // Use status-specific icons
      let statusIcon = ICON.history;
      if (transfer.status.toLowerCase() === 'success') {
        statusIcon = ICON.success;
      } else if (['pending', 'processing', 'initiated', 'awaiting_funds'].includes(transfer.status.toLowerCase())) {
        statusIcon = ICON.loading;
      } else if (['failed', 'canceled'].includes(transfer.status.toLowerCase())) {
...
async function displayPaginationControls(ctx, currentPage, totalItems, hasMore) {
  try {
    console.log(`[HISTORY] Setting up pagination controls: page=${currentPage}, totalItems=${totalItems}, hasMore=${hasMore}`);
    ...
    console.log(`[HISTORY] Current page state: page=${currentPage}, totalPages=${totalPages}`);
    ...
    console.log(`[HISTORY] Has next page: ${hasNextPage} (hasMore=${hasMore}, currentPage=${currentPage}, totalPages=${totalPages})`);
    ...
    console.log(`[HISTORY] Adding page indicator: ${displayPage}/${totalPages}`);
    navRow.push(
      Markup.button.callback(
    ...
    console.log(`[HISTORY] Sending pagination controls with ${navigationButtons.length} rows`);
    await ctx.reply(
      navigationMessage,
      {
        parse_mode: 'Markdown',
    ...
    try {
      await ctx.reply(
        formatSubheader('Return to Menu'),
        {
          parse_mode: 'Markdown',
          reply_markup: {
            inline_keyboard: [[
      ...
      console.error(`[HISTORY] Failed to send fallback navigation:`, fallbackError);
    }
  }
}
...
const pageAction = Composer.action(/^history_page_(\d+)_(.+)$/, authMiddleware(), async (ctx) => {
  try {
    ...
    try {
      ...
      console.warn('[HISTORY] Could not delete loading message:', deleteError);
    }
...
const historyTestCommand = Composer.command('historytest', authMiddleware(), async (ctx) => {
  try {
    await ctx.reply(
      formatLoading('Testing API connection...'),
      { parse_mode: 'Markdown' }
    );
    ...
    console.log('[HISTORYTEST] Starting API test with token:', token.substring(0, 10) + '...');
    ...
    console.log('[HISTORYTEST] API response:', JSON.stringify(response, null, 2));
...
const historyConfigCommand = Composer.command('historyconfig', authMiddleware(), async (ctx) => {
  try {
